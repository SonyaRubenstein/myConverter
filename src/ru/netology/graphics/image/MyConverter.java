...
public class MyConverter implements TextGraphicsConverter {
    ...
    public String convert(String url) throws IOException, BadImageSizeException {
        ...
        double widthRatio = (double) img.getWidth() / img.getHeight();
        if (widthRatio > maxRatio) {
            throw new BadImageSizeException(widthRatio, maxRatio);
        }

        double heightRatio = (double) img.getHeight() / img.getWidth();
...
//        img.getWidth(); //ширина скачанной картинки, пришедшей в программу
//        img.getHeight();//высота скачанной картинки, пришедшей в программу

        int newWidth = 0;
        int newHeight = 0;

        if (img.getHeight() <= maxHeight && img.getWidth() <= maxWidth) { //если вписывается в поле
            newHeight = img.getHeight(); // программа ничего не делает с картинкой
            newWidth = img.getWidth();
        } else {
            if ((img.getWidth() / maxWidth) >= (img.getHeight() / maxHeight)) { //100/150 (0,6) >= 90/25 (3,6)
                //если соотношение по ширине у нас больше чем соотношение по высоте (константа)
                //scaleRatio - потенциальная константа для уменьшения, вычисленная
                //  либо на основании не вписывания по высоте, либо не вписывания  по ширише
                double scaleRatio = ((double) img.getWidth() / maxWidth);
                newWidth = Math.round((float) (img.getWidth() / scaleRatio));
                newHeight = Math.round((float) (img.getHeight() / scaleRatio));
            } else { //иначе
                double scaleRatio = ((double) img.getHeight() / maxHeight);
                newWidth = Math.round((float) (img.getWidth() / scaleRatio));
                newHeight = Math.round((float) (img.getHeight() / scaleRatio));
            }
        }
        ...
        Image scaledImage = img.getScaledInstance(newWidth, newHeight, BufferedImage.SCALE_SMOOTH);//SCALE_SMOOTH - плавное сужение
        ...
        BufferedImage bwImg = new BufferedImage(newWidth, newHeight, BufferedImage.TYPE_BYTE_GRAY); // новая пустая картинка
        ...
        int[] tmpArray = new int[3]; //временный
        char[][] charColorArray = new char[newWidth][newHeight];


        for (int w = 0; w < newWidth; w++) { //сначала бежим циклом по ширине картинки
            for (int h = 0; h < newHeight; h++) { //потом - по высоте
                int color = bwRaster.getPixel(w, h, tmpArray)[0]; //возвращает заполненный массив интов
                char c = schema.convert(color); //включается в работу TextColorSchema
                charColorArray[w][h] = c; //запоминаем символ c, например, в двумерном массиве или как-то ещё на ваше усмотрение
            }
        }
        ...
        // от схемы.
        StringBuilder charColorString = new StringBuilder(); //собирает все переменные для цвета
        for (int h = 0; h < newHeight; h++) { // бегаем по высоте
            for (int w = 0; w < newWidth; w++) {

                charColorString.append(charColorArray[w][h])
                        .append(charColorArray[w][h]); //по два символа, чтобы не делать вывод в консоль узким

            }
            charColorString.append("\n");
        }
        return charColorString.toString(); // Возвращаем собранный текст.
    }

    @Override
    public void setMaxWidth(int width) { //это как разрешение экрана
        this.maxWidth = width;
    }
